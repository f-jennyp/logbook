...
<table class="table table-bordered table-hover table-striped">
    ...
    <Tr class="active">
        ...
        <th>
            ...
            if ($currentSort == 'time' && $currentOrder == 'asc') {
                echo "<a href=\"?page=display_record&sort=time&order=desc\">TIME ▼</a>";
            } else {
                echo "<a href=\"?page=display_record&sort=time&order=asc\">TIME ▲</a>";
            }
            ?>
        </th>
    ...
    $sort = isset($_GET['sort']) ? $_GET['sort'] : 'date'; // Default sorting by 'date' if not specified
    $order = isset($_GET['order']) ? $_GET['order'] : 'asc'; // Default sorting order is ascending

    $sql = "SELECT * FROM attendance ORDER BY $sort $order, date DESC LIMIT $offset, $rec_limit";
